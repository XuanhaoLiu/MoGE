...
# in_channels     : the number of frequency bands
# hidden_channels : the number of hidden layers channels
# num_points      : the number of EEG channels
...
class Moe_GCN_Transformer_unit(nn.Module):
    ...
    def forward(self, x, A):
        ...
        y = rearrange(y, 'N T (V C) -> N T V C', C=self.num_points)
        ...
        for e in range(self.num_experts):
            mask = (idx==e).float()
            mask = mask.repeat(1, 1, V).reshape(y.shape)
            z.append(torch.mul(y, mask))
        
        y = rearrange(y, 'N T V C -> N T C V')
        y = self.res(y)
        for e in range(self.num_experts):
            yg = self.GCNs[e](z[e], A)
            mask = (idx==e).float()
            yg = torch.mul(yg, mask.repeat(1, 1, V).reshape(N, T, C, V))
        ...
        y = y.squeeze(1)
        y = rearrange(y, '(N V) C T -> N T (V C)', V=V)
        y = self.squ(y)
...
class Sparse_MoGE(nn.Module):
    def __init__(self, in_channels, hidden_channels, num_points, time_window, num_layers, heads, dim_head, num_classes, num_experts=6, pool='cls'):
        ...
        self.layers = nn.ModuleList([
            Moe_GCN_Transformer_unit(hidden_channels, hidden_channels, num_points, heads, dim_head, num_experts)
...
if __name__ == "__main__":
    model = Sparse_MoGE(in_channels=5, hidden_channels=64, num_points=62, time_window=5, num_layers=1, heads=2, dim_head=4, num_classes=3, num_experts=3, pool='cls')
    # x.shape = N T V C
    # N: batch size
    # T: time window
    # V: frequency bands' number
    # C: channel number
